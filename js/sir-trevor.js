    ...
    SirTrevor.Blocks.Listunordered = SirTrevor.Block.extend({
      ...
      loadData: function(data) {
        ...
        if (_.isArray(data)) {
          return _.each(data, function(item) {
            _this.renderNewItem(item);
            _this.renderGalleryItem(item);
            return _this.renderNewVideo(item);
          });
        }
      },
      onContentPasted: function(e) {
        var input, parentBlock, val;
  
        input = $(e.target);
        parentBlock = input.parent('.gallery-item');
        val = input.val();
        return this.handleDropPaste(parentBlock, val);
      },
      handleDropPaste: function(block, url) {
        var blockData, videos;
        ...
        if (_.isURI(url)) {
          if (url.indexOf('youtu' !== -1 || url.indexOf('vimeo' !== -1))) {
            ...
            } else if (videos[1] !== void 0) {
              data.video.source = videos[1];
              data.video.remote_id = videos[2];
            }
            if (blockData.video.source === 'youtu') {
              blockData.video.source = 'youtube';
            }
